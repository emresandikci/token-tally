...
function mapProvider(raw: string | undefined): Provider {
  ...
  if (r.includes("openai") || r === "azure" || r.includes("azure_openai") || r.includes("text-completion-openai")) return "openai";
...
export function transformLiteLLM(raw: Record<string, unknown>): Record<string, ModelPricing> {
  const out: Record<string, ModelPricing> = {};
  for (const [name, entryRaw] of Object.entries(raw)) {
    if (name === "sample_spec" || !entryRaw || typeof entryRaw !== "object") continue;
    const e = entryRaw as RawLiteLLMEntry;
    if (typeof e.input_cost_per_token !== "number" || typeof e.output_cost_per_token !== "number") {
      continue;
    }
    if (e.mode && !["chat", "completion", "responses"].includes(e.mode)) continue;
    out[name] = {
...
export async function fetchLiteLLMPrices(timeoutMs = 10_000): Promise<Record<string, ModelPricing>> {
